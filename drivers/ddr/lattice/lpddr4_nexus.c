...
LPDDR_RET lpddr4_init(lpddr4 *instance_ptr, unsigned int base_addr){
	...
	volatile status_reg_t sts_reg;
	volatile clk_chng_t clk_chng_reg;
	...
	do{
		clk_chng_reg=(clk_chng_t)lpddr4_read32(lpddr4_inst->lpddr4_base_address,CLK_CHANGE_REG);
	...
	sts_reg = (status_reg_t)lpddr4_read32(lpddr4_inst->lpddr4_base_address,STATUS_REG);
...
/**
 * @brief : lpddr4_TemperatureChangeInterruptEnable - This API is used to enable temperature change interrupt.
 * 			The Memory Controller periodically reads MR4(Mode Register 4) and according to the 'Temperature Check Period' attribute,
 * 			This interrupt bit asserts when MR4 indicates a temperature	change, and the refresh rate is not equal to 1x refresh.
 * 			Refer IP user guide for a register description.
 * @param : instance_ptr [In] - Handle of the lpddr4 structure.
 * @return : Success or Failure.
 */
unsigned int lpddr4_TemperatureChangeInterruptEnable(lpddr4 *InstancePtr){
	...
	en_reg.fields.temp_change_int=1;
	lpddr4_write32(InstancePtr->lpddr4_base_address,INT_ENABLE_REG,en_reg.reg);
	return SUCCESS;
}

/**
 * @brief : lpddr4_TemperatureChangeInterruptDisable - This API is used to disable temperature change interrupt.
 * 			Refer IP user guide for a register description.
 * @param : instance_ptr [In] - Handle of the lpddr4 structure.
 * @return : Success or Failure.
 */
unsigned int lpddr4_TemperatureChangeInterruptDisable(lpddr4 *InstancePtr){
	...
	int_enable_reg_t en_reg;
	en_reg = (int_enable_reg_t)lpddr4_read32(InstancePtr->lpddr4_base_address, INT_ENABLE_REG);
	en_reg.fields.temp_change_int=0;
	lpddr4_write32(InstancePtr->lpddr4_base_address,INT_ENABLE_REG,en_reg.reg);
	return SUCCESS;
}

/**
 * @brief : lpddr4_TrainingErrorInterruptEnable - This API is used to enable training error interrupt.
 * 			This Interrupt bit asserts when the Training Engine encounters an error during training. The
 * 			user should read STATUS_REG to determine the specific error.
 * 			Refer IP user guide for a register description.
 * @param : instance_ptr [In] - Handle of the lpddr4 structure.
 * @return : Success or Failure.
 */
...
/**
 * @brief : lpddr4_TrainingErrorInterruptDisable - This API is used to disable training error interrupt.
 * 			In this case, The user can poll STATUS_REG to determine the specific error.
 * 			Refer IP user guide for a register description.
 * @param : instance_ptr [In] - Handle of the lpddr4 structure.
 * @return : Success or Failure.
 */
...
/**
 * @brief : lpddr4_GetStatusReg - This API is used to read Status Register (STATUS_REG).
 * 			Refer IP user guide for a register description.
 * @param : instance_ptr [In] - Handle of the lpddr4 structure.
 * 			reg_data [Out] - Value read from register.
 * @return : Success or Failure.
